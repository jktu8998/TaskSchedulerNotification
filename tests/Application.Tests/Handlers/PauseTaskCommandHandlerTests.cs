...
public class PauseTaskCommandHandlerTests
{
    ...
    [Fact]
    public async Task HandleAsync_PauseScheduledTask_Success()
    {
        // Arrange
        ...
        var command = new PauseTaskCommand(taskId);
    ...
    [Fact]
    public async Task HandleAsync_TaskNotInScheduled_ThrowsInvalidOperationException()
    {
        ...
        // Транзакция началась, но откатилась из-за доменного исключения
        _unitOfWorkMock.Verify(u => u.BeginTransactionAsync(It.Is<CancellationToken>(ct => ct == _ct)), Times.Once);
